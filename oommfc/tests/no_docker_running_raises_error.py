...
def test_exception_is_raised_if_no_OOMMF():
    ...
    # The next test only makes sense if we have no docker deamon
    # running.  So let's check for that.
    ...
    # would be good to check for RuntimeError to be raised if docker
    # is not running, but don't know how to force that. Expect
    # exception as docker executable doesn't exist:
    dockername = "docker"
    dockerimage = "joommf/oommf"
    oommf = oc.OOMMF(varname="OOMMFTCL", dockername=dockername,
                     dockerimage=dockerimage, where="docker")
